//Generar una matriz de 4 filas y 5 columnas con números aleatorios entre 1 y 100, y hacer su matriz transpuesta.
/**
 * 
 */

/**
 * @author usuario
 *
 */
public class eje37 {

	/**
	 * @param args
	 */
	public static void main(String[] args) {
		// TODO Auto-generated method stub
		int generar[][];
		generar=new int [4][5];
		for (int fila = 0; fila < generar.length; fila++) {//creamos las filas de una matriz
			for (int columna = 0; columna < generar[0].length; columna++) {//creamos la comunas
				generar [fila][columna]=(int) ((Math.random()*100)+1);
				System.out.printf("% 03d",generar[fila][columna]);
			}
			System.out.println();

		}
		System.out.println();

		//convertir las filas en columnas para trasponer la matriz	
		for (int columna = 0; columna < generar[0].length; columna++) {
			for (int fila = 0; fila < generar.length; fila++) {
				System.out.printf("% 03d",generar[fila][columna]);	
			}
			System.out.println();	
		}


	}

}
